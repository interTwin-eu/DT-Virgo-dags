...
class Preprocess(Trainer):
    def __init__(self,params: YamlWrapper,conf:YamlWrapper,whiteconf:YamlWrapper,save:Optional[bool] = False,source:str='',
    ...
    def execute(self,datalist:Optional[TFrame]=None) -> int:
        ...
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                 transformed= qtransform(batch.to(device=device)).detach().cpu() 
        ...
        if nev%batch_size==1:
            print('Squeezing last element...')
            print(qtransform_list[-1].shape)
            last=qtransform_list[-1].squeeze(1)
            qtransform_list[-1]=last.unsqueeze(0)
