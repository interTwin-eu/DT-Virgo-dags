...
def train_decoder(num_epochs, 
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for i, batch in enumerate(dataloader):
            ...
            input_tensor = batch[:, 1:].to(device)

            optimizer.zero_grad()  # Zero the gradients
            generated = generator(input_tensor.float())  # Forward pass
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                target = batch[:, 0].unsqueeze(1).to(device).float()
                input_tensor = batch[:, 1:].to(device)
                generated = generator(input_tensor.float())
        ...
        if epoch % nacc == 0 and (not(os.path.isfile("./conf/accuracy.lock"))):
            ...
            for batch in back_loader:
                background_post = generate_data(generator, batch.detach().cpu()/norm).to('cpu')
