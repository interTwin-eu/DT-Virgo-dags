...
class GlitchTrainer(TorchTrainer):
    def __init__(
        self,
        num_epochs: int = 2,#number of epochs
        coptim_betas: List=[0.9,0.999], # optimizer beta parameters
        ...
        checkpoint_path: str = "checkpoints/epoch_{}.pth",#checkpoint path
        temp_path: str= "./temp/",#where to save data for (eg. data for inference)
        ...
        output_channels: int=1,#number of model output channel
    ...
    def train(self):
        ...
        for denoise_acc1, veto_acc1 in zip(denoise_acc,veto_acc):                
                        self.log(item=denoise_acc1,identifier='Denoising accuracy',kind='metric',step=logstep*self.acc_freq)
                        self.log(item=veto_acc1,identifier='Veto Accuracy',kind='metric',step=logstep*self.acc_freq)
                        logstep+=1                  
                                          
        
        
        checkpoint=torch.load(self.checkpoints_location.format('best'))
        self.generator_2d.load_state_dict(checkpoint['model_state_dict'])
        self.log(
    ...
    def execute(
        ...
        print('Test ', dataset[2].shape)
        print('Background', dataset[1].shape)
        print('Means')
        print(dataset[5])
